...
namespace OpenNos.GameObject.Event.GAMES
{
    /*public static class SheepGame
    {
        ...
        public static void GenerateSheepGames()
        {
            ...
            */
            ...
            ServerManager.Instance.Broadcast(UserInterfaceHelper.GenerateMsg(Language.Instance.GetMessageFromKey("SHEEPGAME_STARTED"), 1));
            ServerManager.Instance.Broadcast("qnaml 4 #guri^514 Do you want to participate in the sheep raid?");
            ServerManager.Instance.EventInWaiting = true;
            Thread.Sleep(30 * 1000);
            ServerManager.Instance.Sessions.Where(s => s.Character?.IsWaitingForEvent == false).ToList().ForEach(s => s.SendPacket("esf 1"));
            ServerManager.Instance.EventInWaiting = false;
        ...
        public class SheepThread
        {
            ...
            public void Run(MapInstance map)
            {
                ...
                foreach (ClientSession session in _map.Sessions)
                {
                    ...
                    {
                        session.SendPacket("say 1 -1 10 In 10 seconds The sheep farm raid begins soon.");
                    });
                    Observable.Timer(TimeSpan.FromSeconds(35)).Subscribe(o =>
                    {
                        session.SendPacket("say 1 -1 10 In 5 seconds The sheep farm raid begins soon.");
                    });
                    Observable.Timer(TimeSpan.FromSeconds(36)).Subscribe(o =>
                    {
                        session.SendPacket("say 1 -1 10 In 4 seconds The sheep farm raid begins soon.");
                    });
                    Observable.Timer(TimeSpan.FromSeconds(37)).Subscribe(o =>
                    {
                        session.SendPacket("say 1 -1 10 In 3 seconds The sheep farm raid begins soon.");
                    });
                    Observable.Timer(TimeSpan.FromSeconds(38)).Subscribe(o =>
                    {
                        session.SendPacket("say 1 -1 10 In 2 seconds The sheep farm raid begins soon.");
                    });
                    Observable.Timer(TimeSpan.FromSeconds(39)).Subscribe(o =>
                    {
                        session.SendPacket("say 1 -1 10 In 1 seconds The sheep farm raid begins soon.");
            ...
            private void End(Character character)
            {
                ...
                character.SpAdditionPoint = character.SpAdditionPoint > 1000000 ? 1000000 : character.SpAdditionPoint;
    ...
    class SheepGame
    {
        ...
        public const int MiniPlayerForStart = 2;

        public static void GenerateSheepGames()
        {
            Thread.Sleep(5 * 1000);
            ServerManager.Instance.Broadcast(UserInterfaceHelper.GenerateMsg(Language.Instance.GetMessageFromKey("SHEEP_STARTED"), 1));
            ServerManager.Instance.Broadcast("qnaml 4 #guri^514 The Sheep Game just started! Join now !");
            ServerManager.Instance.EventInWaiting = true;
            Thread.Sleep(30 * 1000);
            ServerManager.Instance.Sessions.Where(s => s.Character?.IsWaitingForEvent == false).ToList().ForEach(s => s.SendPacket("esf 1"));
            ServerManager.Instance.EventInWaiting = false;
        ...
        public class SheepThread
        {
            ...
            public void Run(MapInstance map)
            {
                ...
                foreach (ClientSession session in _map.Sessions)
                {
                    ...
                    Spawn = true;
                    session.Character.CanAttackSh = true;
                    session.Character.Speed = 5;
                    session.Character.IsVehicled = true;
                    session.Character.IsCustomSpeed = true;
                    session.Character.Morph = 1009;
            ...
            {
                character.GenerateFamilyXp(character.Level * 400);
                character.Gold += character.Level * 1000;
                character.GiftAdd(5951, 1);
                character.SpAdditionPoint += character.Level * 1090;
                character.SpAdditionPoint = character.SpAdditionPoint > 1000000 ? 1000000 : character.SpAdditionPoint;
                ...
                character.CanAttackSh = false;
                character.IsCustomSpeed = false;
                character.RemoveVehicle();
                Spawn = false;
                _map.Broadcast("srlst 4");
                _map.Broadcast("srlst 1");
                ServerManager.Instance.ChangeMap(character.Session.Character.CharacterId, character.Session.Character.MapId, character.Session.Character.MapX, character.Session.Character.MapY);
            }
