    ...
    public class Inventory : ThreadSafeSortedList<Guid, ItemInstance>
    {
        ...
        private byte GetMaxSlot(InventoryType pocket)
        {
            ...
            return (byte)(pocket switch
            {
                ...
                _ => Owner.Session != null && Owner.Session.Character.HaveBigBackPack() ? ServerManager.Instance.Configuration.BackpackSize + Expensions[pocket] : 48 + Expensions[pocket] + ((Owner.Session.Character.HaveBackpack() ? 1 : 0) * 12)
            });
        }
