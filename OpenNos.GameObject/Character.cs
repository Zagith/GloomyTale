...
namespace OpenNos.GameObject
{
    public class Character : CharacterDTO
    {
        ...
        public string GenerateEq()
        {
            ...
            return $"eq {CharacterId} {(Invisible && Authority >= AuthorityType.TMOD ? 6 : Undercover ? (byte)AuthorityType.User : Authority < AuthorityType.User ? (byte)AuthorityType.User : Authority >= AuthorityType.TMOD ? 2 : (byte)Authority)} {(byte)Gender} {(byte)HairStyle} {color} {(byte)Class} {GenerateEqListForPacket()} {(!InvisibleGm ? GenerateEqRareUpgradeForPacket() : null)}";
        ...
        public string GenerateGidx() => Family != null && FamilyCharacter != null ? $"gidx 1 {CharacterId} {Family.FamilyId} {Family.Name}({Language.Instance.GetMessageFromKey(FamilyCharacter.Authority.ToString().ToUpper())}) {Family.FamilyLevel} 0|0|0" : $"gidx 1 {CharacterId} -1 - 0 0|0|0";
                ...
                Observable.Timer(TimeSpan.FromMilliseconds(500)).Subscribe(o =>
                {
                    if (Session.HasCurrentMapInstance)
                    {
                        if (CharacterId == dropOwner && StaticBonusList.Any(s => s.StaticBonusType == StaticBonusType.AutoLoot) || MapId == 9305)
                        {
                            ...
                            Session.SendPacket(GenerateSay($"{Language.Instance.GetMessageFromKey("ITEM_ACQUIRED")}: {ServerManager.GetItem(drop.ItemVNum).Name[Session.Account.Language]} x {drop.Amount}{(multiplier > 1 ? $" + {(int)(drop.Amount * multiplier) - drop.Amount}" : "")}", 12));
                            Session.SendPacket(GenerateGold());
                        }
                        else
                        {
                                    ...
                                    if (Session.CurrentMapInstance != null)
                                    {
                                        ...
                                        else
                                        {
                                            if (group?.GroupType == GroupType.Group)
                                            {
                                                ...
                                                {
                                                    group.Sessions.ForEach(s => s.SendPacket(s.Character.GenerateSay(string.Format(Language.Instance.GetMessageFromKey("DROPPED_ITEM"), ServerManager.GetItem(drop.ItemVNum).Name[Session.Account.Language], drop.Amount), 10)));
                                                }
                                            }
                            ...
                            if (Session.CurrentMapInstance != null)
                            {
                                    ...
                                    foreach (BattleEntity damager in damagers)
                                    {
                                        if (!alreadyGifted.Contains(damager.MapEntityId))
                                        {
                                            ...
                                            if (session != null)
                                            {
                                                ...
                                                session.SendPacket(session.Character.GenerateSay($"{Language.Instance.GetMessageFromKey("ITEM_ACQUIRED")}: {ServerManager.GetItem(drop2.ItemVNum).Name[Session.Account.Language]} x {drop2.Amount}{(multiplier > 1 ? $" + {(int)(drop2.Amount * multiplier) - drop2.Amount}" : "")}", 10));
                                ...
                                else
                                {
                                    if (group != null && MapInstance.MapInstanceType != MapInstanceType.LodInstance)
                                    {
                                        ...
                                        {
                                            group.Sessions.ForEach(s => s.SendPacket(s.Character.GenerateSay(string.Format(Language.Instance.GetMessageFromKey("DROPPED_ITEM"), ServerManager.GetItem(drop2.ItemVNum).Name[Session.Account.Language], drop2.Amount), 10)));
                                        }
                                    }
        ...
        public void GetGold(long val, bool isQuest = false)
        {
            ...
            Session.SendPacket(isQuest
                 ? GenerateSay($"Quest reward: [ {ServerManager.GetItem(1046).Name[Session.Account.Language]} x {val} ]", 10)
                 : Session.Character.GenerateSay($"{Language.Instance.GetMessageFromKey("ITEM_ACQUIRED")}: {ServerManager.GetItem(1046).Name[Session.Account.Language]} x {val}", 10));
            Session.SendPacket(Session.Character.GenerateGold());
        }
        ...
        private void GenerateXp(MapMonster monster, Dictionary<BattleEntity, long> damageList = null, bool isGroupMember = false)
        {
            ...
            if (!Session.Account.PenaltyLogs.Any(s => s.Penalty == PenaltyType.BlockExp && s.DateEnd > DateTime.Now))
            {
                ...
                double shellXp = (1 +  (ShellEffectMain.FirstOrDefault(s => s.Effect == (byte)ShellWeaponEffectType.GainMoreXP)?.Value / 100D)) ?? 1;
                int xp = (int)(GetXP(monster, grp) * expDamageRate * (isMonsterOwner ? 1 : 0.8f) * (1 + (
                    GetBuff(CardType.Item, (byte)AdditionalTypes.Item.EXPIncreased)[0] / 100D)) * (1 + (
                        GetBuff(CardType.MartialArts, (byte)AdditionalTypes.MartialArts.IncreaseBattleAndJobExperience)[0] / 100))
                        * shellXp);
                ...
                if (fairy != null)
                {
                    ...
                    while (fairy.XP >= experience)
                    {
                        ...
                        if (fairy.ElementRate + fairy.Item.ElementRate == fairy.Item.MaxElementRate)
                        {
                            ...
                            Session.SendPacket(UserInterfaceHelper.GenerateMsg(string.Format(Language.Instance.GetMessageFromKey("FAIRYMAX"), fairy.Item.Name[Session.Account.Language]), 10));
                        }
                        else
                        {
                            Session.SendPacket(UserInterfaceHelper.GenerateMsg(string.Format(Language.Instance.GetMessageFromKey("FAIRY_LEVELUP"), fairy.Item.Name[Session.Account.Language]), 10));
                        }
