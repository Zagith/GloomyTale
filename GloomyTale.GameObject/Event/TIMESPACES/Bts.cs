...
namespace OpenNos.GameObject.Event.TIMESPACES
{
    public static class Bts
    {
        ...
        public static void GenerateBTS(int Vnum, ClientSession host)
        {
                    ...
                    for (int d = 0; d < 60; d++)
                    {
                        if (!mapinstance.Item1.Monsters.Any(s => s.CurrentHp > 0))
                        {
                            ...
                            foreach (ClientSession cli in mapinstance.Item1.Sessions.Where(s => s.Character != null).ToList())
                            {
                                ...
                                cli.Character.Gold += cli.Character.Level * 10000 * (mapinstance.Item1.Sessions.Count());
                                ...
                                cli.SendPacket(cli.Character.GenerateSay(string.Format(Language.Instance.GetMessageFromKey("WIN_MONEY"), cli.Character.Level * 10000 * (mapinstance.Item1.Sessions.Count())), 10));
