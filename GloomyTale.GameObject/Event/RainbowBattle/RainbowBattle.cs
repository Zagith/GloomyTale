...
namespace GloomyTale.GameObject.Event.RainbowBattle
{
    public static class RainbowBattle
    {
        ...
        public static void GenerateRainbowBattle()
        {
            ...
            int i = 0;
            byte y = 0;
            List<RainbowBattleTeam> arcobalenoTeam = new List<RainbowBattleTeam>();
            RainbowBattleTeam arcobaleno = new RainbowBattleTeam();
            arcobaleno.Member = new List<RainbowBattleMember>();
            arcobaleno.Session = new List<ClientSession>();
            foreach (RainbowBattleMember s in sessions.OrderBy(s => s.GroupId))
            {
                arcobaleno.Member.Add(s);
                arcobaleno.Session.Add(s.Session);
                if (LastGroupId != s.GroupId)
                {
                    if (arcobaleno.Member.Count() > 1)
                    {
                        if (arcobaleno != null)
                        {
                            arcobaleno.Color = LastColor == 1 ? RainbowBattleTeamType.RED : RainbowBattleTeamType.BLUE;
                            arcobaleno.Points = 0;
                            LastColor = arcobaleno.Color == RainbowBattleTeamType.BLUE ? 1 : 0;
                            arcobalenoTeam.Add(arcobaleno);
                            i++;
                            y++;
                            if (i == 2)
                            {
                                map = ServerManager.GenerateMapInstance(2010, MapInstanceType.BAInstance, new InstanceBag());
                                maps.Add(new Tuple<MapInstance, byte>(map, y));
                                i = 0;
                                foreach (RainbowBattleTeam at in arcobalenoTeam)
                                {
                                    int w = 0;
                        ...
                        arcobaleno = new RainbowBattleTeam();
                        arcobaleno.Member = new List<RainbowBattleMember>();
                        arcobaleno.Session = new List<ClientSession>();
                    }
                }
