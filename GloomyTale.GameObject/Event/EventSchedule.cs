...
namespace OpenNos.GameObject
{
    public class EventSchedule //: IConfigurationSectionHandler
    {
        ...
        private static Schedule getSchedule(XmlNode str)
        {
            ...
            {
                int lvlBracket = 0;
                if (str.Attributes["lvlbracket"] != null)
                {
                    lvlBracket = int.Parse(str.Attributes["lvlbracket"].Value);
                }
                string dayOfWeek = "";
                if (str.Attributes["dayOfWeek"] != null)
                {
                    dayOfWeek = str.Attributes["dayOfWeek"].Value;
                }
                return new Schedule
                {
