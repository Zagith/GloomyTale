...
namespace GloomyTale.GameObject.Event.TRUEORFALSE
{
        ...
        public void Run()
        {
            ...
            if (TorFEvent._map.Sessions.Count() > 0)
            {
                foreach (ClientSession s in TorFEvent._map.Sessions)
                {
                    ...
                    s.Character.Gold = s.Character.Gold > ServerManager.Instance.MaxGold ? ServerManager.Instance.MaxGold : s.Character.Gold;
