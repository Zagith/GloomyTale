...
{
    [PacketHeader("$GoldRate", PassNonParseablePacket = true, Authority = AuthorityType.GA)]
    public class GoldRatePacket
    {
        ...
        public static void HandlePacket(object session, string packet)
        {
            if (session is ClientSession sess)
            {
                ...
                GoldRatePacket packetDefinition = new GoldRatePacket();
        ...
        public static void Register() => PacketFacility.AddHandler(typeof(GoldRatePacket), HandlePacket, ReturnHelp);
        ...
        private void ExecuteHandler(ClientSession session)
        {
            if (_isParsed)
            {
                ...
                if (Value <= 1000)
                {
                    ServerManager.Instance.Configuration.RateGold = Value;
                    session.SendPacket(
