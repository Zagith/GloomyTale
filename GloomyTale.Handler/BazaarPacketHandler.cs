    ...
    public class BazaarPacketHandler : IPacketHandler
    {
        ...
        public void GetBazaar(CScalcPacket cScalcPacket)
        {
            ...
            if (bazaarItemDTO != null)
            {
                ...
                {
                    if (Session.Character.Gold + price <= 1000000000 /*ServerManager.Instance.Configuration.MaxGold*/)
                    {
                        Session.Character.Gold += price;
                        Session.SendPacket(Session.Character.GenerateGold());
                    ...
                    else
                    {
                        ...
                        Session.SendPacket(UserInterfaceHelper.GenerateBazarRecollect(bazaarItemDTO.Price, 0, bazaarItemDTO.Amount, 0, 0, name));
                    }
                }
                else
                {
        ...
        public void ModPriceBazaar(CModPacket cModPacket)
        {
            ...
            if (bz != null)
            {
                ...
                StaticBonusDTO medal = Session.Character.StaticBonusList.Find(s =>
                    s.StaticBonusType == StaticBonusType.BazaarMedalGold
                    || s.StaticBonusType == StaticBonusType.BazaarMedalSilver);
                if (cModPacket.Price >= (medal == null ? 1000000 : 1000000000 /*ServerManager.Instance.Configuration.MaxGold*/))
                {
                    Session.SendPacket(
                        UserInterfaceHelper.GenerateMsg(Language.Instance.GetMessageFromKey("PRICE_EXCEEDED"), 0));
                    return;
