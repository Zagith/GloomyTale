            ...
            if (packet != null && !Session.Character.IsSeal)
            {
                ...
                if (packet.Level > 0)
                {
                    Session.Character.Level = packet.Level; //Math.Min(,
                        //100 /*ServerManager.Instance.Configuration.MaxLevel*/);
...
#warning TODO Get Online Characters
        ...
        public void Rarify(RarifyPacket packet)
        {
            if (packet != null)
            {
                ...
                if (packet.Slot >= 0)
                {
                    var wearableInstance = Session.Character.Inventory.LoadBySlotAndType(packet.Slot, 0);
...
#warning Session.SendPacket(Session.Character.GenerateSay("----- SESSION -----", 13));
