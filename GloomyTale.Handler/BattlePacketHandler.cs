...
namespace GloomyTale.Handler
{
    public class BattlePacketHandler : IPacketHandler
    {
        ...
        public void UseSkill(UseSkillPacket useSkillPacket)
        {
            ...
            {
                if (useSkillPacket.UserType == VisualType.Monster)
                {
                        ...
                        case 1619:
                            {
                                Observable.Timer(TimeSpan.FromMilliseconds(5500)).Subscribe(o =>
                                {
                                    ...
                                    target.CurrentMapInstance.Broadcast(StaticPacketHelper.SkillUsed(VisualType.Player, hitRequest.Session.Character.CharacterId, 1,
        ...
        private void TargetHit(int castingId, VisualType targetType, int targetId, bool isPvp = false)
        {
                                ...
                                if (playerToAttack != null && !IceBreaker.FrozenPlayers.Contains(playerToAttack))
                                {
                                            ...
                                            }) <= ski.Skill.Range + 5)
                                    {
                                        ...
                                        if (ski.Skill.HitType == 3)
                                        {
                                            ...
                                            //foreach (long id in Session.Character.MTListTargetQueue.Where(s => s.EntityType == VisualTypeUserType.Player).Select(s => s.TargetId))
                                            foreach (long id in Session.Character.GetMTListTargetQueue_QuickFix(ski, VisualType.Player))
                                            {
                                ...
                                if (monsterToAttack != null)
                                {
                                        ...
                                        new MapCell { X = monsterToAttack.MapX, Y = monsterToAttack.MapY }) <= ski.Skill.Range + 5 + monsterToAttack.Monster.BasicArea)
                                    {
                                        ...
                                        if (ski.Skill.HitType == 3)
                                        {
                                            ...
                                            //foreach (long id in Session.Character.MTListTargetQueue.Where(s => s.EntityType == VisualTypeUserType.Monster).Select(s => s.TargetId))
                                            foreach (long id in Session.Character.GetMTListTargetQueue_QuickFix(ski, VisualType.Monster))
                                            {
            ...
            if (characterSkill.CanBeUsed())
            {
                ...
                if (Session.Character.Mp >= mpCost && Session.Character.Hp > hpCost && Session.HasCurrentMapInstance)
                {
                    ...
                    Observable.Timer(TimeSpan.FromMilliseconds(characterSkill.Skill.CastTime * 100)).Subscribe(o =>
                    {
                        ...
                        //foreach (long id in Session.Character.MTListTargetQueue.Where(s => s.EntityType == VisualTypeUserType.Player).Select(s => s.TargetId))
                        foreach (long id in Session.Character.GetMTListTargetQueue_QuickFix(characterSkill, VisualType.Player))
                        {
                            ClientSession character = ServerManager.Instance.GetSessionByCharacterId(id);
